...
>(({ className, ...props }, ref) => {
  const [sliderValue, setSliderValue] = React.useState<number[]>([50]); // Default value at 50

  const handleValueChange = (value: number[]) => {
    setSliderValue(value); // Update state
    // The parent component (existingKnowledge) will handle the logic when slider value changes
  };
  ...
  return (
    <SliderPrimitive.Root
      ...
      value={sliderValue}
      onValueChange={handleValueChange} // Track value change here
      {...props}
    >
